	    ...
	    try {
	      ...
	      for (int cont=0;cont<lista.size();cont++) {
	    	  ...
	    	  if (tabla.equals("Profesor")) {
	    		  ...
	    		  stmt.executeUpdate(consulta,PreparedStatement.RETURN_GENERATED_KEYS);
	    		 ResultSet rs= stmt.getGeneratedKeys();
	    		 rs.next();
	    		 idprof=rs.getString(1);
	    	  } else if (tabla.equals("Asignatura")) {
	    		  if (idprof!="0") {
	    		  consulta="INSERT INTO " +split[0] + "(CodAsignatura, Nombre,IdProfesor) VALUES ('"+split[1] +"','"+split[2] +"','"+idprof+"');";
